...
public:
	...
	bool IsBalancedCore(TreeNode *root, int &depth) {
		...
		if (IsBalancedCore(root->left, leftDepth) && IsBalancedCore(root->right, rightDepth)) {
...
// ====================测试代码====================
void Test(string testName, TreeNode* root, bool expected)
{
    printf("%s begins: ", testName.c_str());

    Solution s = Solution();
    if(s.IsBalanced(root) == expected)
        printf("Passed.\n");
    else
        printf("Failed.\n");
}
